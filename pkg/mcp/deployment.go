	...
	tools := []server.ServerTool{
		{
			Tool: mcp.NewTool("deployment_analyze",
				...
				mcp.WithString("namespace",
					mcp.Description("the namespace to analyze deployments in, can be a \"\" string to all namespace"),
				),
			),
...
func (s *Server) deploymentScale(ctx context.Context, ctr mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if replicas < 0 {
		return mcp.NewToolResultText("scale deployment replicas error"), nil
	}
	res, err := s.k8s.DeploymentScale(ctx, ns, deploy, replicas)
	if err != nil {
		return mcp.NewToolResultText(fmt.Sprintf("failed to scale deployment %s/%s: %v", ns, deploy, err)), nil
	}
...
func (s *Server) deploymentAnalyze(ctx context.Context, ctr mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		return mcp.NewToolResultText(fmt.Sprintf("failed to analyze deployments in namespace %s: %v", ns, err)), nil
	}
	return mcp.NewToolResultText(res), nil
}
